...
async def receive_data_task(client):
    ...
    while True:
        try:
            ...
            print(response)
            button_str, button_str2, button_str3, button_str4, pwm_str = response.split(",")
            button_state = int(button_str)
            button_state2 = int(button_str2)
            button_state3 = int(button_str3)
            button_state4 = int(button_str4)
            pwm_value = float(pwm_str)
            pwm.value = pwm_value
            pwm2.value = pwm_value
            if button_state2 == 1:
                robot.backward()
            elif button_state == 1:
                robot.forward()
            elif button_state3 == 1:
                robot.left()
            elif button_state4 == 1:
                robot.right()
            else:
                robot.stop()
